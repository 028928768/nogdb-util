...
class App extends Component {
  constructor(props){
    ...
    this.setFlagtoAddCanvas = this.setFlagtoAddCanvas.bind(this);
    this.handleAddEdgeToDatabase = this.handleAddEdgeToDatabase.bind(this);
    this.setFlagEdgetoCanvas = this.setFlagEdgetoCanvas.bind(this);
    this.setFlagEdgetoDatabase = this.setFlagEdgetoDatabase.bind(this);
    ...
    setFlagEdgetoDatabase = () =>{
      this.setState({
        flagisEdgetoCanvas:false
      })
    }
    handleChange(e){
      this.setState({
    ...
    handleShowIncomingClick() {      
      const edgeList = this.queryIncomingEdges(this.state.nodeID)

      this.addEdgesToCanvas(edgeList)
    ...
    handleAddEdgeToDatabase(edges){
      let newEdge;
      if(this.state.flagisEdgetoCanvas === true){
        newEdge = { from: this.state.srcvalue, to: this.state.dscvalue }
      } else if (this.state.flagisEdgetoCanvas === false){
        newEdge =[{ from: "1", to: "2" },{ from: "20", to: "20" }]//state
        newEdge = edges;
      }
